# ai-cmux-orchestrator (`aico`)

`ai-cmux-orchestrator` is a cMUX AI workspace orchestrator. The short command is `aico`.

## Purpose

Run from any project directory, `aico` makes Codex the base orchestrator tab with Amit's `cxscb` launcher and coordinates optional kid-named side panes in the same cMUX workspace. The base orchestrator agent is selectable per launch with `--agent claude|codex|devin` (shorthands `--claude`, `--codex`, `--devin`; default `codex`); kid panes and their launch commands never change with this flag. Claude and the extra Codex panel are enabled by default as `kid-claude` and `kid-codex`; Devin is disabled by default and only becomes AICO-managed as `kid-devin` when `AICO_CREATE_DEVIN_PANEL=true`. AICO suppresses Codex Apps/external MCP startup for the base orchestrator and the optional Codex panel, reuses existing enabled managed panes when present, retitles legacy `Claude`/`Codex`/`Devin` panes to their kid-prefixed names, relaunches the agent CLI in any reused pane that has dropped back to an idle shell, creates only missing enabled panes, and starts a durable AICO poller. The poller checks enabled Claude/Codex/Devin panes every 60 seconds, records meaningful events, nudges the base Codex orchestrator with a safe control envelope, and handles Claude usage-limit auto-resume.
...
1. Outside cMUX, attempt one handoff:
   ...
   ```zsh
   cmux new-workspace --name "${PWD:t}" --cwd "$PWD" --focus true --command aico
   ```

2. Inside cMUX, rename the current tab to `codex`.
3. Read the cMUX tree and preserve existing side-agent surfaces. AICO manages only panes whose feature flag is enabled; disabled panes are left untouched and unregistered.
4. Fill missing layout pieces only:
   ...
   - When a lower enabled pane already exists and an upper enabled pane is missing, AICO splits `up` from the lower pane to restore stack order without closing anything.
...
7. Register enabled managed surfaces in durable state and ensure the AICO watcher daemon is running.
8. Pass stable cMUX workspace/surface IDs into the base Codex orchestrator prompt.
9. Teach the base Codex orchestrator that Amit's exact bare message `Reset` means run `aico --reset`. Reset checks enabled managed side panes, refuses if any enabled agent has active or unresolved work, otherwise creates one fresh `terminal` surface and closes the base orchestrator plus enabled AICO-managed AI surfaces.
...
## Manager-loop guardrails

The orchestrator prompt also enforces manager-style discipline:

- Real cmux CLI only: never invent commands or flags; on error consult `cmux --help` instead of guessing.
- Live-state inspection: check `cmux --json tree --workspace <id>` and `cmux list-panes` before acting on any pane; trust the live tree over stale assumptions.
- Human-led planning: for multi-chunk or ambiguous work, show Amit a one-glance plan (chunks, target kid panes, acceptance criteria) and wait for a go-ahead before the first delegation. Confirmation is skipped only for a single obvious chunk or when Amit pre-approved proceeding.
- Anti-drift polling: after delegating, periodically re-check `cmux --json tree` and read active kid panes so workers never drift or stall silently.
- Panes over tab groups: tab groups are easy to lose; managed tabs keep their `kid-<agent>` titles via `cmux rename-tab` so roles stay findable.
...
- AICO tailors the prompt to the kid agent's command style:
...
```env
AICO_CREATE_CLAUDE_PANEL=true
AICO_CREATE_CODEX_PANEL=true
AICO_CREATE_DEVIN_PANEL=false
```

- `AICO_CREATE_CLAUDE_PANEL=true` (current default): create/reuse/poll/reset/route Claude. Set `false` to leave existing Claude panes untouched and omit Claude routing/auto-resume registration for this workspace.
- `AICO_CREATE_CODEX_PANEL=true` (current default): create/reuse/poll/reset/route the extra side Codex panel titled `kid-codex`. Set `false` to leave existing Codex side panes untouched. The base orchestrator still runs.
- `AICO_CREATE_DEVIN_PANEL=false` (current default): do not create, reuse, poll, reset, or route Devin. Existing Devin panes are left untouched. Set `true` to opt into Devin.
...
```zsh
aico                       # default: Codex orchestrator (cxscb)
aico --agent claude        # Claude orchestrator (clscb)
aico --claude              # shorthand for --agent claude
aico --agent devin "task"  # Devin orchestrator (devin --permission-mode dangerous -- <prompt>)
aico--claude "task"        # global shorthand functions from aliases.zsh: aico--claude / aico--codex / aico--devin
```

The flag only swaps the process the orchestrator prompt is handed to. The base tab keeps the title `codex` for every agent because reset and the AICO daemon locate the base tab by that exact title. An invalid `--agent` value exits 2 before any cMUX changes.

## Codex MCP suppression

AICO launches the Codex orchestrator with:

```zsh
cxscb --disable apps -c 'mcp_servers={}' <orchestrator prompt>
```

This keeps AICO from initializing Codex Apps (`codex_apps`) or user-configured external MCP servers. The optional Codex side panel uses the same suppressed `cxscb` command. This does not change the Claude launcher or the optional Devin launcher.

Additional watcher controls:

```env
AICO_CLAUDE_AUTO_RESUME_DAEMON=true   # default; set false to avoid auto-starting the watcher
AICO_DEVIN_POLL_DAEMON=true            # default; set false to disable AICO event polling inside the watcher
AICO_STATE_DIR=~/.local/state/ai-cmux-orchestrator
```

`AICO_STATE_DIR` defaults to `$XDG_STATE_HOME/ai-cmux-orchestrator` or `~/.local/state/ai-cmux-orchestrator`.

## Codex side panel routing

When `AICO_CREATE_CODEX_PANEL=true`, AICO creates an extra side pane titled `kid-codex` underneath `kid-claude` with:

```zsh
zsh -lc "cd <cwd> && cxscb --disable apps -c 'mcp_servers={}'"
```

The lowercase `codex` tab remains the base orchestrator. The `kid-codex` pane is the side Codex agent. The base orchestrator prompt tells future AICO sessions that "ask Codex" / "send to Codex" means use this side pane, repair only that pane when explicitly requested, and never treat the daemon inbox as a user request.

## Devin boil panel routing

Only when `AICO_CREATE_DEVIN_PANEL=true`, AICO creates the `kid-devin` pane below `kid-codex` when the Codex panel is enabled, below `kid-claude` when the Codex panel is disabled, or right of the base orchestrator when Devin is the only enabled side pane:

```zsh
zsh -lc 'cd <cwd> && dey.boil'
```

`dey.boil` is Amit's Devin launcher for boil-the-ocean mode. The Codex orchestrator prompt also tells future AICO sessions that "ask Devin" / "send to Devin" means:
...
## AICO daemon poller and event inbox

The same detached AICO watcher polls enabled managed panes every 60 seconds. On bootstrap, AICO persists:
...
Events are stored in `devin-poll.json` as the AICO event inbox. The JSONL user-facing form is sanitized: no raw Claude, Codex-panel, or optional Devin screen excerpt is emitted into the base Codex orchestrator. Each line includes `summary`, `state`, `severity`, IDs, and an `excerpt_hash`.
...
3. Discovers enabled AICO-managed terminal surfaces titled `kid-claude`, `kid-codex`, and/or `kid-devin`. Disabled panes are not read or closed.
...
7. Creates one fresh terminal surface in the base Codex pane, focuses it, renames it `terminal`, stops the AICO watcher daemon, then closes the base Codex orchestrator plus enabled managed Claude/Codex/Devin surfaces.
...
Durable state lives at `claude-auto-resume.json` in `AICO_STATE_DIR` and records:
...
5. Logs recovery events in `aico --status` output
...
- `ai-cmux-orchestrator` — Bun executable entrypoint.
- `bin/aico` — short zsh wrapper.
- `bin/ai-cmux-orchestrator` — full-name zsh wrapper.
- `environment.env` — daemon feature flags; Claude and Codex panels default to enabled, Devin defaults disabled.
- `src/ai-cmux-orchestrator/claude-auto-resume.ts` — durable Claude usage-limit detector, scheduler, retry loop, state store, and status formatter.
- `src/ai-cmux-orchestrator/devin-poll.ts` — durable AICO event inbox, enabled Claude/Codex/Devin meaningful-state detector, base Codex control-notice sender, state store, and status formatter.
- `src/ai-cmux-orchestrator/reset.ts` — exact `Reset` detector, enabled Claude/Codex/Devin active-work guard, basic-terminal creator, and AICO AI surface closer.
- `src/ai-cmux-orchestrator/watcher-daemon.ts` — combined foreground watcher loop for Claude auto-resume and Devin polling.
- `src/ai-cmux-orchestrator/` — daemon-specific TypeScript source.
...
```zsh
aico
ai-cmux-orchestrator --help
AICO_CREATE_DEVIN_PANEL=true aico "ask Claude, Codex, and Devin to inspect the auth flow"
aico --status
aico --events --unread
aico --reset
aico Reset
...
bun run typecheck
zsh -n bin/aico
zsh -n bin/ai-cmux-orchestrator
bun ai-cmux-orchestrator --help
bun ai-cmux-orchestrator --status
bun ai-cmux-orchestrator --events --unread
```
