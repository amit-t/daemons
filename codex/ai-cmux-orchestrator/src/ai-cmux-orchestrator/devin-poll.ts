...
export const DEVIN_POLL_STATE_DIR = "AICO_STATE_DIR";
export const DEVIN_POLL_DEFAULT_LINES = 200;
export const AICO_DAEMON_NOTICE_VERSION = "AICO_DAEMON_NOTICE_V1";
...
export function devinPollStateDirectory(env: Record<string, string | undefined> = process.env): string {
  return env[DEVIN_POLL_STATE_DIR] || join(env.XDG_STATE_HOME || join(homedir(), ".local", "state"), "ai-cmux-orchestrator");
}
...
export async function registerDevinPollFromOrchestratorContext(
...
function createAgentInboxEvent(options: {
  ...
  const event: DevinPollEvent = {
    id: `${options.at}:aico_event:${options.agent}:${options.surfaceId}:${options.state.events.length}`.replace(/[^A-Za-z0-9_.:-]+/g, "_"),
    type: "aico_event",
    at: options.at,
...
export function formatUnreadDevinPollEventsJsonl(
  ...
  const unread = state.events.filter((event) => event.type === "aico_event" && !event.readAt);
...
export function formatDevinPollSitrep(state: DevinPollState): string {
  const unreadCount = state.events.filter((event) => event.type === "aico_event" && !event.readAt).length;
  const lines: string[] = ["AICO event poll status", `Unread AICO events: ${unreadCount}`];
  if (!state.registrations.length) lines.push("No AICO event poll surfaces registered.");
  for (const registration of state.registrations) {
    ...
    lines.push(`Registered AICO event watcher: workspace ${registration.workspaceId}, ${claudeStatus}, ${codexStatus}, ${devinStatus}, base Codex surface ${registration.orchestratorSurfaceId}.`);
  ...
  for (const event of state.events.filter((candidate) => candidate.type === "aico_event").slice(-5)) {
