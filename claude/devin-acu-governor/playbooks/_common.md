...
11. **Direct cap headroom ceiling — 500 hard max, ≤ 250 preferred.** In every flow (`set-limits`, `set-limits-new`, `boost`, `over`, new-cycle resets, ad hoc `all commands` work), no user's direct cap may sit more than **500 ACUs above their current consumed ACUs**. Prefer recommending **≤ 250 ACUs** of headroom; treat 250–500 as needs-justification. Exceeding 500 requires an explicit in-session override from the user, and even then print a warning naming the policy before and after the write. The jq programs enforce this (`max_headroom`, default 500) — never hand-raise a cap past what they output.
12. **Borrow donor safety + presentation.** Never present donor reductions as "negative ACUs." Borrow donors give positive ACUs; preview as `cap_before → cap_after` plus positive `given`. Reject any plan that would put a donor below current consumed ACUs or below the configured donor buffer/floor. If a preview looks like negative ACUs to the user, find cleaner donors instead, preferring high-surplus donors over many 1-ACU skim donors while preserving the zero-sum invariant unless the user explicitly accepts pool-headroom overage risk.
