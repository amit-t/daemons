...
5. **Their Local Agent limit.** GET `/v3beta1/enterprise/users/{user_id}/consumption/acu-limits`. Report explicit `local_agent.cycle_acu_limit` override and `billing_org_id` if present. Also GET `/v3beta1/enterprise/users/consumption/acu-limits` for the default per-user limit so you can state the effective per-user limit when no override exists. Read the donor record too (`donor_record` in Run context; live only when its `cycle_start` equals the current cycle's `after`): if this user has a live donor entry, report `baseline_cap`, `given_total`, and each reduction — a high consumed/cap ratio here may be `DONOR` state (cap reduced by DAG, low real usage), not genuine pressure.
