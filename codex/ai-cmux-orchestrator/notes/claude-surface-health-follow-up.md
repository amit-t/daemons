# Follow-up: AICO Claude surface health guard
...
Expected AICO behavior: if the Claude tab is not actually running Claude Code, AICO should close that stale tab and start a fresh Claude tab with `clscb` before routing prompts.

## Requested daemon change

Update `codex/ai-cmux-orchestrator` so Claude routing and/or bootstrap does not trust tab title alone.
...
4. Keep safety boundaries:
   ...
   - Log recovery action in `aico --status` output or recent events.

## Suggested code areas

- `src/ai-cmux-orchestrator/orchestrator.ts`
  - `createAgentSurface`
  - `closeManagedAgentSurfaces`
  - orchestrator prompt text that tells Codex how to route Claude messages
- `src/ai-cmux-orchestrator/claude-auto-resume.ts`
  - `resolveClaudeSurface`
  - `readClaudeScreen`
  - `isPlausibleClaudeSurface`
- `test/ai-cmux-orchestrator-setup.test.ts`
...
2. AICO closes that stale surface and launches a new `clscb` surface.
3. Prompt routing uses the new surface ID, not the stale one.
4. Healthy Claude Code UI remains untouched.
5. `aico --status` reports recovery event.
...
Implemented in `src/ai-cmux-orchestrator/claude-auto-resume.ts` via `sendClaudePromptWithHealthGuard`, `isPlausibleClaudeSurface`, recovery events, and status output. Bootstrap behavior remains separate: AICO preserves existing Claude/Devin panes and only creates missing panes.
