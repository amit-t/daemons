...
| `src/components/UserTable.tsx` | User cap table: text search, status + cap-source filters, sortable columns, billing org, projected cycle-end ACUs plus an under/close/over forecast badge vs the allocated cap (`—` on pre-forecast snapshots), explicit Copy button beside each email, and a Details action button that opens the detail drawer without row-hover/click side effects |
| `src/components/UserDetail.tsx` | Per-user drawer: daily ACU line chart over the cycle (cap-pace reference line), projected-at-cycle-end card with run rate + forecast badge, Devin Cloud session stats, model + surface (IDE) bar lists from Windsurf analytics, product split; the header email is a click-to-copy token; closes on Esc/✕/backdrop |
