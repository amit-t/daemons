...
- Model-pinned launcher profiles are also selectable before the command: `--co` launches Claude Opus through `co`, `--cf` launches Claude Fable through `cf`, `--deo` launches Devin Opus through `deo`, `--def` launches Devin Fable through `def`, `--des`/`--del`/`--det` launch Devin GPT-5.6 Sol/Luna/Terra through `des`/`del`/`det`, and `--dey` launches default-model Devin through `dey`. These are direct profile flags; canonical `--agent` values remain `claude`, `codex`, and `devin` only. `--defu` launches the cheapest enabled Devin Fusion pair through `DEFU_YOLO=1 defu`; the environment form enables dangerous mode in the standalone binary without forwarding its wrapper-only `--yolo` flag through the Profiles `defu` function, which is already dangerous. Defu owns dynamic pair selection plus its precision, boil-the-ocean, and caveman prompt, while DAG supplies the unchanged command playbook after `--`.
- Codex pins from Profiles `zsh_functions/codex-models.zsh` are selectable the same way: `--cxl`, `--cxs`, `--cxa` launch Luna/Sol/Astra (precision + SCB) at base reasoning, and an `l`/`m`/`h`/`u` suffix selects low/medium/high/ultra reasoning (`--cxll` … `--cxau`, 15 pins). `--cxm` runs the interactive `CXM` picker (choose model + reasoning, saved as Codex defaults) and then launches the playbook. Codex takes the prompt as a bare positional, so no `--` is added. Unsupported reasoning fails without launching (for example `--cxlu`: Luna has no ultra), with no silent fallback.
...
dag --co status
dag --cf status
dag --cxsh boost.org "Platform Eng"   # Codex Sol, high reasoning
dag --deo status
dag --def status
dag --des status
dag --del boost over
...
7. Also fetch each active user's `run_rate` (last-7-day average ACUs/day) and the cycle's `days_left`. Run `lib/compute-caps.jq` on `{pool, days_left, users:[{user_id,email,consumed,run_rate,member,active}]}`; the jq program filters out `member:false` and `active:false`.
8. Read current user overrides with `GET /v3beta1/enterprise/users/{user_id}/consumption/acu-limits`.
9. Preview email, user_id, consumed, old override, new cap, delta, `projected` (cycle-end forecast), Σ caps vs pool, excluded users, stale excluded-cap cleanup rows, UI instruction — plus a "will not survive the cycle at even share — boost candidates" list of every user whose `projected` exceeds their proposed cap.
...
4. If the target is uncapped, run `lib/borrow-caps.jq` with `recipients` containing only that target and `donors` containing active capped users (with `run_rate`/`days_left` and, when the enterprise projection is under pool, a `forecast` block — see the Borrow math below). Donors are ranked highest-safe-surplus-first, then lowest consumption, and never cut below the `donor_buffer`/`min_donor_headroom`/`min_donor_cap_after` floor; donors without `run_rate` are excluded from lending by default (`require_forecast`).
5. Preview one target cap plus donor reductions, `forecast_headroom`/`forecast_funded` when a forecast block was used, and proving `zero_sum: true`/`sum_before == sum_after` when `forecast_funded == 0` (otherwise `sum_after == sum_before + forecast_funded`, and the plan is NOT zero-sum).
...
- **Donors** = active current-member users whose cap **is** set, ranked highest safe surplus first, then lowest consumption. They lend cap headroom. Recipients are never donors; no donor is cut below its consumed ACUs + 10% buffer, `min_donor_headroom` (25) above its projected consumption, or `min_donor_cap_after` (50) — jq built-in backstops per hard rule 11. Donors without a `run_rate` are excluded from lending by default (`require_forecast`).
...
5. Also fetch each donor's `run_rate` (last-7-day average) and the cycle's `days_left`, and — when the enterprise linear projection is under `DAG_MONTHLY_ACU_POOL` — the live `projected_cycle_total`. Run `lib/borrow-caps.jq` on `{donor_buffer, max_headroom, min_donor_cap_after, min_donor_headroom, require_forecast, days_left, forecast?, recipients:[{user_id,email,consumed,member,active}], donors:[{user_id,email,cap,consumed,run_rate,member,active}]}`; the jq program filters out `member:false` and `active:false`.
6. Preview the recipient caps, donor Borrow table, `forecast_headroom`/`forecast_funded`, excluded inactive/former users (plus any `no_run_rate_forecast` donor exclusions), and stale excluded-cap cleanup rows; the output proves `sum_before == sum_after` (`zero_sum: true`) for the active eligible set only when `forecast_funded == 0` — with forecast funding, `zero_sum` is `false` and the preview shows the Σ-caps growth instead.
...
Borrow math (`lib/borrow-caps.jq`), zero-sum when no forecast funding is drawn:
- `even_share` — donor + forecast headroom is ample: `cap_i = floor(consumed_i) + min(share, max_headroom)`, `share = floor((Σ donor_headroom + forecast_headroom − Σ floor(consumed)) / N)` (≥ 1). The playbook passes `max_headroom: 250` by default; 250–500 only on explicit in-session user request; above 500 never — the jq built-in default of 500 is the backstop clamp.
...
No recipient is ever capped below its own current consumption. Donor floor: `max(ceil(consumed × (1 + donor_buffer)), ceil(consumed) + min_donor_headroom, min_donor_cap_after)` — `min_donor_headroom` (25) and `min_donor_cap_after` (50) are jq built-in backstops per hard rule 11 and never raise a donor's cap. Donors may carry an optional `run_rate` (last-7-day average ACUs/day, with cycle `days_left` in the input): the donor's protected floor is then also raised to cover projected end-of-cycle consumption — `ceil((consumed + run_rate × days_left) × (1 + donor_buffer))` — so heavy burners with big nominal surplus are protected while long-idle users with little nominal headroom become safe donors. The borrow draws from the highest projected safe surplus first (consumed as tie-break). `require_forecast` (default `true`) excludes any donor lacking `run_rate` from lending entirely — reported in `donors_excluded` with reason `no_run_rate_forecast` — instead of silently falling back to a consumed-only floor; pass `require_forecast: false`, with the reason stated in the preview, to accept consumed-only floors instead.

**Forecast-headroom funding.** When the enterprise linear projection (`consumed / elapsed_days × cycle_days`, same math as the dashboard) is under `DAG_MONTHLY_ACU_POOL`, an optional `forecast: {pool, projected_cycle_total, utilization}` input (utilization 0.5 default, up to 1.0 only on explicit in-session request) funds `forecast_headroom = floor((pool − projected_cycle_total) × utilization)`. Funding is forecast-first: `forecast_funded = min(borrowed, forecast_headroom)` is drawn before any donor gives up cap headroom, so donors only cover `borrowed − forecast_funded`. `zero_sum` is `true` only when `forecast_funded == 0`; whenever forecast funding is used, `sum_after == sum_before + forecast_funded` and the plan grows Σ explicit caps rather than redistributing them.
...
4. Run `lib/boost-plan.jq`:
   ...
   - donor floor = `max(ceil(consumed + donor_buffer × share), ceil(consumed) + min_donor_headroom, min_donor_cap_after)`, raised again to `ceil((consumed + run_rate × days_left) × 1.1)` when the donor's `run_rate` is known — `min_donor_headroom` (25) and `min_donor_cap_after` (50) are jq built-in backstops (hard rule 11); projected heavy burners are protected, long-idle low-headroom users become safe donors;
   - `require_forecast` (default `true`) excludes any donor without `run_rate` from the pool entirely — their consumed-only floor can't be trusted against end-of-cycle burn — listed in `donors_excluded_no_forecast`; pass `require_forecast: false` in-session, with the reason stated in the preview, to accept consumed-only floors instead;
   - when the enterprise linear projection is under `DAG_MONTHLY_ACU_POOL`, an optional `forecast` block (`{pool, projected_cycle_total, utilization}`, utilization 0.5 default) funds `forecast_headroom = floor((pool − projected_cycle_total) × utilization)` **before** any donor is drawn down (rule 14), further clamped by an optional `forecast.remaining` to a shared budget still unspent this run — how `dag boost all` keeps a sequential batch from having every recipient re-spend the same full headroom — the preview shows `forecast_headroom`/`forecast_funded`/`donor_funded` and states the plan is NOT zero-sum whenever `forecast_funded > 0`;
   - output proves `sum_before == sum_after` when fully funded with no forecast funding; with forecast funding, `sum_after − sum_before == forecast_funded`.
...
Verdicts: `401` = **rejected** (the key itself is invalid or revoked; summary prints the keychain store command), `403` = **missing** (key accepted, permission absent), `429` = rate-limited, `000` = unreachable. A `401` on the ACU Limit Write probe is a rejection, not the `403` inconclusive case.
...
| `DAG_LAUNCHER_CX<PIN>` | same-named Profiles function | Codex pin launcher for `--cx<pin>`, e.g. `DAG_LAUNCHER_CXSH` for `--cxsh` (default `cxsh`) |
| `DAG_LAUNCHER_CXM` | `CXM --` | Interactive Codex model picker used by `--cxm`; the prompt follows its `--` |
...
| Every doctor probe `rejected (401 — key invalid or revoked)` | Stored `cog_` key is stale or revoked (common on a second Mac after a key rotation). Store the current key under `devin-cog-key`; `dag setup-extract` on a working Mac prints the command. |
