...
5. **Their soft cap.** From the ledger (path in Run context) if present and fresh; otherwise note no allocation exists and suggest `dag set-limits`.
...
8. **Pointers.** If they're near or over their soft cap, note that `dag boost <email>` can reallocate ACUs to them from low consumers. If a single costly model dominates their burn, say so plainly.
