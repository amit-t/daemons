...
} from "../src/ai-cmux-orchestrator/reset.ts";
...
describe("AICO reset request detection", () => {
...
describe("resetAiCmuxWorkspace", () => {
  ...
  test("creates one fresh terminal surface and closes idle AICO AI surfaces", async () => {
    const { runner, calls } = strictRunnerFor({
    ...
    expect(buildResetStatusText(result)).toContain("AICO reset complete");
  });
  ...
  test("pushes back and does not close anything when the enabled Codex panel is active", async () => {
    ...
    const result = await resetAiCmuxWorkspace({
      env: {
        AICO_CREATE_CLAUDE_PANEL: "false",
  ...
  test("fails safely outside cMUX", async () => {
    ...
    expect(buildResetStatusText(result)).toContain("AICO reset requires cMUX");
  });
});
