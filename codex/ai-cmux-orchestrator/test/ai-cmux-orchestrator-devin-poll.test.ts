...
`);
  ...
  test("registers Devin and Codex surfaces from orchestrator context", async () => {
    ...
    await registerDevinPollFromOrchestratorContext(
      {
        cwd: "/work/project-x",
  ...
  test("does not poll existing Devin panes when Devin panel is disabled", async () => {
    ...
    expect(state.events.filter((event) => event.type === "aico_event" && event.agent === "Devin")).toHaveLength(0);
  });
  ...
  test("does not poll existing Claude or Codex panes when those panels are disabled", async () => {
    ...
    expect(state.events.filter((event) => event.type === "aico_event")).toHaveLength(0);
  });
...
`;
    ...
    expect(formatDevinPollSitrep(state)).toContain("Unread AICO events: 1");
  });
