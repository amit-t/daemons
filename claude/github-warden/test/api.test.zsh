...
work=$(mktemp -d)
export GHW_CURL="/bin/zsh ${script_dir}/fixtures/curl-stub.zsh"
...
EOF
: > "$GHW_STUB_LOG"
ghw_api PUT /orgs/o/memberships/u '{"role":"member"}' > "${work}/out2"; rc=$?
out=$(<"${work}/out2")
...
EOF
ghw_api GET /users/ghost > "${work}/out3"; rc=$?
out=$(<"${work}/out3")
...
ghw_api PUT /orgs/o/memberships/u '{"role":"member"}' > "${work}/out4"; rc=$?
out=$(<"${work}/out4")
...
EOF
ghw_api_paged /orgs/o/members > "${work}/out5"; rc=$?
out=$(<"${work}/out5")
