# AICO Default Kid-Panel Goal Orchestration Design

## Goal

AICO base orchestrators should attempt to decompose ordinary user tasks across enabled `kid-*` panels even when Amit does not explicitly say “distribute” or “orchestrate,” and every delegated kid-panel prompt must tell that panel to mark the assigned work as a goal.

## Current behavior

AICO already creates enabled managed side panes (`kid-claude`, `kid-codex`, optional `kid-devin`) and injects a base orchestrator prompt from `codex/ai-cmux-orchestrator/src/ai-cmux-orchestrator/orchestrator.ts`. Existing prompt rules strongly route requests that explicitly name kid panes, but ordinary tasks may be handled in the base orchestrator or backgrounded.
...
When no managed side-agent panels are enabled, prompt stays honest: the base orchestrator does work itself unless Amit changes feature flags and restarts AICO.

## Files

- Modify `codex/ai-cmux-orchestrator/src/ai-cmux-orchestrator/orchestrator.ts` to extend prompt text.
- Modify `codex/ai-cmux-orchestrator/test/ai-cmux-orchestrator-panels.test.ts` with failing prompt assertions before implementation.
- Modify `codex/ai-cmux-orchestrator/README.md` and root `README.md` to document behavior.
