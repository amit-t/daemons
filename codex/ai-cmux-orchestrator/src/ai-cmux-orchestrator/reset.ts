import { defaultRunner, type CommandResult, type CommandRunner } from "./orchestrator.ts";
...
export async function resetAiCmuxWorkspace(options: AiCmuxResetOptions): Promise<AiCmuxResetResult> {
  ...
  if (!workspaceId) return { mode: "error", message: "AICO reset requires cMUX; CMUX_WORKSPACE_ID is not set." };

  let surfaces: CmuxSurface[];
  try {
    surfaces = await readCmuxTreeSurfaces(runner, workspaceId, windowId);
  } catch (error) {
    return { mode: "error", workspaceId, message: `AICO reset could not inspect cMUX workspace: ${errorMessage(error)}` };
  }
  ...
  if (!codexSurface || !codexSurfaceId || !codexPaneRef) {
    return {
      mode: "error",
      workspaceId,
      message: `AICO reset could not find the Codex surface/pane (${currentSurfaceId || "current surface unknown"}).`,
    };
  }
  ...
  for (const targetId of targetIds) {
    ...
    if (result.code !== 0) {
      return {
        mode: "error",
        workspaceId,
        message: `AICO reset could not close ${targetId}: ${exactFailure("cmux close-surface", result)}`,
      };
    }
...
async function createBasicTerminalSurface(
  ...
  if (create.code !== 0) {
    return { mode: "error", message: `AICO reset could not create a basic terminal: ${exactFailure("cmux new-surface", create)}` };
  }
  ...
  if (!terminalSurfaceId) return { mode: "error", message: `AICO reset could not find the new terminal surface in pane ${paneRef}.` };

  const rename = await runner("cmux", ["rename-tab", "--workspace", workspaceId, ...windowArgs(windowId), "--surface", terminalSurfaceId, "terminal"]);
  if (rename.code !== 0) {
    return { mode: "error", message: `AICO reset could not rename the basic terminal: ${exactFailure("cmux rename-tab", rename)}` };
  }
...
export function buildResetStatusText(result: AiCmuxResetResult): string {
  if (result.mode === "reset") {
    return `AICO reset complete: opened basic terminal ${result.terminalSurfaceId}; closed ${result.closedSurfaceIds.join(", ")}.`;
  }
